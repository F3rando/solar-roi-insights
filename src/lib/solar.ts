...
// Annual production (kWh) ≈ kW * irradiance * 365 * efficiency
export function annualProductionKwh(zone: Zone, kw: number): number {
  const eff = efficiencyPct(zone.heatC) / 100;
  return kw * zone.irradiance * 365 * eff * 0.85; // 0.85 system derate
}

export function annualSolarSavings(zone: Zone, inputs: Inputs): number {
  return annualProductionKwh(zone, inputs.systemKw) * zone.utilityRate;
}

export function paybackYears(zone: Zone, inputs: Inputs): number {
  const yearly = annualSolarSavings(zone, inputs);
...
export function projectedSavings(
  ...
  const annualKwh = annualProductionKwh(zone, inputs.systemKw);
...
export function cumulativeSavings(zone: Zone, inputs: Inputs, years = 25): number {
  const proj = projectedSavings(zone, inputs, years);
  const last = proj[proj.length - 1];
  return last.grid - last.solar;
}

export function co2AvoidedTons(zone: Zone, inputs: Inputs, years = 25): number {
...
export function zoneStatus(zone: Zone, inputs: Inputs): {
  label: "High Efficiency Zone" | "Moderate ROI" | "Waste of Money Zone";
  tone: "solar" | "warn" | "heat";
} {
  const yrs = paybackYears(zone, inputs);
